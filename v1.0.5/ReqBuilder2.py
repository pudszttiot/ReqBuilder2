...
# Define app version
__version__ = "1.0.5"
UPDATE_CHECK_URL = "https://api.github.com/repos/YOUR_GITHUB_USERNAME/YOUR_REPO/releases/latest"
...
class UpdateChecker(QThread):
    ...
    def run(self):
        try:
            ...
            data = response.json()
            latest_version = data["tag_name"].lstrip("v")  # Extract version
            update_url = data["html_url"]  # GitHub release page

            if latest_version > __version__:
                self.update_signal.emit(latest_version, update_url)
        except requests.RequestException:
            pass  # Silent failure if offline
